package play.modules.reactivemongo.validation.collection
...
/**
 * A Collection that interacts with the Play JSON library, using the validation API `Rule` and `Write`.
 */
object `package` {
  type JsRule[T] = Rule[JsValue, T]
  type JsWrite[T] = Write[T, JsValue]

  implicit object JSONCollectionProducer extends GenericCollectionProducer[JsObject, JsRule, JsWrite, JSONCollection] {
    def apply(db: DB, name: String, failoverStrategy: FailoverStrategy) = new JSONCollection(db, name, failoverStrategy)
  }
}

trait JSONGenericHandlers extends GenericHandlers[JsObject, JsRule, JsWrite] {
  ...
  case class BSONStructureReader[T](rule: JsRule[T]) extends GenericReader[JsObject, T] {
    def read(doc: JsObject) = rule.validate(doc) match {
      case success: Success[JsValue, T] => success.get
      case error: Failure[T, JsValue]   => throw new NoSuchElementException(error.toString)
    }
  }
  case class BSONStructureWriter[T](writer: JsWrite[T]) extends GenericWriter[T, JsObject] {
    def write(t: T) = writer.writes(t).as[JsObject]
  }
  def StructureReader[T](rule: JsRule[T]) = BSONStructureReader(rule)
  def StructureWriter[T](writer: JsWrite[T]): GenericWriter[T, JsObject] = BSONStructureWriter(writer)
}

object JSONGenericHandlers extends JSONGenericHandlers

case class JSONDocumentReaderAsBufferReader[T](rule: JsRule[T]) extends BufferReader[T] {
  def read(buffer: ReadableBuffer) = rule.validate(JSONGenericHandlers.StructureBufferReader.read(buffer)).get
}
...
case class JSONCollection(
    db: DB,
    name: String,
    failoverStrategy: FailoverStrategy) extends GenericCollection[JsObject, JsRule, JsWrite] with JSONGenericHandlers with CollectionMetaCommands {
  def genericQueryBuilder: GenericQueryBuilder[JsObject, JsRule, JsWrite] =
   ...
   * @param writeConcern the [[reactivemongo.core.commands.GetLastError]] command message to send in order to control how the document is inserted. Defaults to GetLastError().
   */
  def save[T](doc: T, writeConcern: GetLastError = GetLastError())(implicit ec: ExecutionContext, writer: JsWrite[T]): Future[LastError] =
...
case class JSONQueryBuilder(
    ...
    options: QueryOpts = QueryOpts()) extends GenericQueryBuilder[JsObject, JsRule, JsWrite] with JSONGenericHandlers {
  ...
  object structureReader extends JsRule[JsObject] {
    private lazy val rule: JsRule[JsObject] = Rule.fromMapping {
      case obj @ JsObject(_) => Success(obj)
      case _ => Failure(Seq(ValidationError("error.expected.json")))
    }

    def validate(json: JsValue) = rule.validate(json)
  }

  protected def toStructure[T](writer: JsWrite[T], subject: T) = writer.writes(subject)

  def convert[T](rule: JsRule[T]): BufferReader[T] = JSONDocumentReaderAsBufferReader(rule)
